...
namespace Lab_2_For_Lab_2
{
        public class Calculaitons
        {
            ...
            {
                if (boxMessage == "")
                    message = "Ошибка!Заполните поле";
                else
                {
                    num = Convert.ToInt32(boxMessage);

                    if (num <= 0)//Введенное число не может быть <=0
                        message = "Ошибка!Значение не может быть равно 0 или быть отрицательным"; //Выводим сообщение об ошибке
                }
            }
            catch (System.FormatException)
            {
                message = "Ошибка!Введены символы: " + boxMessage + ". Требуется ввести цифры, значение которых >0"; //Пользователь ввел символы
            }
